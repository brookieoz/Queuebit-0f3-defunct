...
public struct QbitSnapshotData : ISnapshotData<QbitSnapshotData>
{
    public uint tick;
    private int QbitDataComponentPlayerId;
    ...
    public int GetQbitDataComponentPlayerId(GhostDeserializerState deserializerState)
    {
        return (int)QbitDataComponentPlayerId;
    }
    public int GetQbitDataComponentPlayerId()
    {
        return (int)QbitDataComponentPlayerId;
    }
    public void SetQbitDataComponentPlayerId(int val, GhostSerializerState serializerState)
    {
        QbitDataComponentPlayerId = (int)val;
    }
    public void SetQbitDataComponentPlayerId(int val)
    {
        QbitDataComponentPlayerId = (int)val;
    }
    ...
    public void PredictDelta(uint tick, ref QbitSnapshotData baseline1, ref QbitSnapshotData baseline2)
    {
        var predictor = new GhostDeltaPredictor(tick, this.tick, baseline1.tick, baseline2.tick);
        QbitDataComponentPlayerId = predictor.PredictInt(QbitDataComponentPlayerId, baseline1.QbitDataComponentPlayerId, baseline2.QbitDataComponentPlayerId);
    ...
    public void Serialize(int networkId, ref QbitSnapshotData baseline, DataStreamWriter writer, NetworkCompressionModel compressionModel)
    {
        changeMask0 = (QbitDataComponentPlayerId != baseline.QbitDataComponentPlayerId) ? 1u : 0;
        ...
        if ((changeMask0 & (1 << 0)) != 0)
            writer.WritePackedIntDelta(QbitDataComponentPlayerId, baseline.QbitDataComponentPlayerId, compressionModel);
    ...
    public void Deserialize(uint tick, ref QbitSnapshotData baseline, DataStreamReader reader, ref DataStreamReader.Context ctx,
        NetworkCompressionModel compressionModel)
    {
        this.tick = tick;
        changeMask0 = reader.ReadPackedUIntDelta(ref ctx, baseline.changeMask0, compressionModel);
        if ((changeMask0 & (1 << 0)) != 0)
            QbitDataComponentPlayerId = reader.ReadPackedIntDelta(ref ctx, baseline.QbitDataComponentPlayerId, compressionModel);
        else
            QbitDataComponentPlayerId = baseline.QbitDataComponentPlayerId;
        if ((changeMask0 & (1 << 1)) != 0)
        {
    ...
    public void Interpolate(ref QbitSnapshotData target, float factor)
    {
